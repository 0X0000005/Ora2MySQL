...
**注意事项**：
- 确保在 push 前已经测试过编译
- 确认 commit message 清晰准确
- 如果添加了新功能，记得更新版本号

#### 6.5 版本标签管理

**重要**：如果本次修改包含版本号更新，需要创建并推送 Git Tag！

**检查版本号是否变化：**
```bash
# 查看 web.go 中的版本号是否已修改
git diff HEAD~1 HEAD -- internal/server/web.go | grep "const Version"
```
...
// turbo
```bash
# 创建带注释的标签（替换 vX.X.X 为实际版本号，例如 v1.6.4）
...
**标签命名规范**：
- Tag 名称必须与代码中 `internal/server/web.go` 的版本号一致
- 使用 `-a` 创建带注释的标签（推荐）
- Tag 注释应包含本次版本的主要更新内容

**注意事项**：
- 确保代码已经推送后再推送 Tag
- 如果标签已存在，需先删除：`git tag -d vX.X.X && git push origin :refs/tags/vX.X.X`
- 推送所有标签：`git push --tags`（不推荐，容易误推送）
...
版本号位置：`internal/server/web.go` 中的 `Version` 常量
...
- [ ] git 提交包含清晰的说明
- [ ] git push 到远程仓库
- [ ] 如果版本号有变化，已创建并推送 Git Tag
- [ ] 创建了完成报告

- 小改动/修复：补丁版本 +1（v1.2.0 → v1.2.1）
...
git commit -m "chore: 更新版本号至vX.X.X"
git push
```
...
- [ ] 单元测试通过
- [ ] 手动测试验证功能正确
- [ ] 使用 build.bat 完整编译（而不是 go build）
- [ ] git 提交包含清晰的说明
- [ ] git push 到远程仓库
- [ ] 创建了完成报告
...
### Q: 必须用 build.bat 吗？
A: 是的！build.bat 会编译 Windows 和 Linux 两个版本，并使用 UPX 压缩。直接用 go build 只会生成当前平台的版本。

### Q: 如何跳过某些步骤？
A: 根据实际需求调整，但建议至少执行：修改 → 测试 → 编译（build.bat）→ 提交 → push → Tag（如有版本变化）

### Q: Git Tag 推送失败怎么办？
A: 常见问题有：
- **标签已存在**：先删除旧标签 `git tag -d vX.X.X && git push origin :refs/tags/vX.X.X`，然后重新创建
- **权限问题**：确保有远程仓库的推送权限
- **网络问题**：检查网络连接，重试推送

### Q: 忘记推送 Tag 怎么办？
A: 随时可以补推：`git tag -a vX.X.X <commit-hash> -m "Release vX.X.X" && git push origin vX.X.X`
